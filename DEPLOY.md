...
Ce guide explique comment déployer Naruto Mythos Game sur VPS.app gratuitement avec support WebSocket.

## Prérequis

1. Un compte [VPS.app](https://vps.app) (gratuit)
...
```bash
# Installer VPS CLI (optionnel mais recommandé)
npm install -g @vps/cli
...
Via le dashboard VPS:
1. Va sur https://vps.app/dashboard
...
Dans l'onglet "Variables" de ton service VPS, ajoute :

```env
DATABASE_URL=mongodb+srv://dlkdigitalagency_db_user:***REDACTED***@cluster0.q4izgvf.mongodb.net/naruto-mythos-tcg?appName=Cluster0
...
VPS détectera automatiquement le `Dockerfile` et déploiera l'application.

### 5. Configurer le domaine personnalisé

1. Dans VPS, va dans l'onglet "Settings" de ton service
2. Section "Public Networking"
3. Clique sur "+ Custom Domain"
4. Ajoute `naruto.daikicorp.fr`
5. VPS te donnera un CNAME à ajouter dans ta configuration DNS chez ton registrar

Exemple de record DNS à ajouter :
```
Type: CNAME
Name: naruto
Value: [le-domaine-vps-fourni]
TTL: 3600
```
...
2. Vérifie les logs VPS pour les erreurs CORS
3. Assure-toi que le domaine est correctement configuré dans les variables

### Erreur de connexion MongoDB

1. Vérifie que l'IP de VPS est whitelistée dans MongoDB Atlas
2. Dans MongoDB Atlas > Network Access > Add IP Address > Allow Access from Anywhere (0.0.0.0/0) pour tester

### Problèmes de build

Vérifie les logs de build dans VPS. Si besoin, redéploie avec "Clear Cache and Redeploy".

## Limites du plan gratuit VPS
...
Si VPS ne convient pas, Render.com offre aussi un plan gratuit avec WebSocket :
