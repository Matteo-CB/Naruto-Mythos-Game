...
async function fetchFlags(names: string[]): Promise<void> {
  ...
  try {
    ...
    if (res.ok) {
      const data = (await res.json()) as { flags?: Record<string, string | null> };
      for (const n of toFetch) flagCache.set(n, data.flags?.[n] ?? null);
    } else {
      for (const n of toFetch) flagCache.set(n, null);
    }
  } catch {
