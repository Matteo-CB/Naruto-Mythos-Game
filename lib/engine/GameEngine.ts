...
import { resynchroniserLesOptionsDeMain } from '../effects/handOptions';
...
export class GameEngine {
  ...
  static applyAction(state: GameState, player: PlayerID, action: GameAction): GameState {
    ...
    let result = resynchroniserLesOptionsDeMain(
      suivreEntreesEnJeu(state, reparerPilesVides(releaseDanglingControl(enforceAttachmentConditions(rescueOrphanedAttachments(state, inner))))),
    );
  ...
  static handlePendingAction(state: GameState, player: PlayerID, action: GameAction): GameState {
    ...
    if (action.type === 'DECLINE_OPTIONAL_EFFECT') {
      ...
      if (effect.targetSelectionType === 'SS130_CONFIRM_DUEL_MODIFIER') {
        newState.pendingEffects.splice(effectIdx, 1);
        newState.pendingActions = newState.pendingActions.filter((a) => a.sourceEffectId !== effect.id);
        return EffectEngine.queueOrochimaru130Target(newState, effect, false);
      }
