...
describe('Shikamaru 022 — AMBUSH no-target bug repro', () => {
  ...
  it('finds the opponent character even when they played in the previous round (cross-round ambush)', () => {
    const enemy: CharacterInPlay = mockCharInPlay(
      { instanceId: 'hinata-1', controlledBy: 'player2', originalOwner: 'player2' },
      { id: 'KS-030-C', name_fr: 'HINATA HYÛGA', power: 2, chakra: 2 },
    );
    const shikamaruHidden: CharacterInPlay = mockCharInPlay(
      { instanceId: 'shika-1', isHidden: true, wasRevealedAtLeastOnce: false },
      { id: 'KS-022-UC', name_fr: 'SHIKAMARU NARA', power: 3, chakra: 3 },
    ...
    state.log = [
      ...
      createLogEntry(1, 'action', 'PLAY_HIDDEN', 'p1 plays Shikamaru hidden', 'player1',
        'game.log.playHidden', { mission: 1, instanceId: 'shika-1' }),
      createLogEntry(1, 'action', 'PLAY_CHARACTER', 'p2 plays HINATA HYÛGA', 'player2',
        'game.log.playCharacter',
        { card: 'HINATA HYÛGA', mission: 1, cost: 2 }),
