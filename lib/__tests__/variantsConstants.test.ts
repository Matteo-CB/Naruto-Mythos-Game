...
describe('variants constants', () => {
  ...
  it('probabilities sum to 1', () => {
    const sum =
      (VARIANT_PACK_PROBABILITIES.L ?? 0) +
      (VARIANT_PACK_PROBABILITIES.SV ?? 0) +
      (VARIANT_PACK_PROBABILITIES.MV ?? 0) +
      (VARIANT_PACK_PROBABILITIES.RA ?? 0) +
      (VARIANT_PACK_PROBABILITIES.HOLO_C ?? 0) +
      (VARIANT_PACK_PROBABILITIES.HOLO_UC ?? 0);
    expect(sum).toBeCloseTo(1, 10);
  });

  it('rarest rarities have lowest probability', () => {
    expect((VARIANT_PACK_PROBABILITIES.SV ?? 0)).toBeLessThan((VARIANT_PACK_PROBABILITIES.L ?? 0));
    expect((VARIANT_PACK_PROBABILITIES.L ?? 0)).toBeLessThan((VARIANT_PACK_PROBABILITIES.MV ?? 0));
    expect((VARIANT_PACK_PROBABILITIES.MV ?? 0)).toBeLessThan((VARIANT_PACK_PROBABILITIES.RA ?? 0));
    expect((VARIANT_PACK_PROBABILITIES.RA ?? 0)).toBeLessThan((VARIANT_PACK_PROBABILITIES.HOLO_UC ?? 0));
    expect((VARIANT_PACK_PROBABILITIES.HOLO_UC ?? 0)).toBeLessThan((VARIANT_PACK_PROBABILITIES.HOLO_C ?? 0));
  });
