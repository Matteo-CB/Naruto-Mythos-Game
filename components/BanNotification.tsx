...
export function BanNotification() {
  ...
  if (chatBan && !dismissed.has('chatBan')) {
    ...
    items.push({
      key: 'chatBan',
      content: (
        <span className="text-[11px]" style={{ color: 'var(--t-danger)' }}>
    ...
    items.push({
      ...
      content: (
        <div className="flex flex-col gap-0.5">
          <span className="text-[11px]" style={{ color: 'var(--t-text)' }}>
            {t('reportResolved', { target: notif.targetName })}
          </span>
          {notif.action && notif.action !== 'dismiss' && (
            <span className="text-[10px]" style={{ color: 'var(--t-success)' }}>
              {t('actionTaken', { action: notif.action })}
            </span>
          )}
          {notif.action === 'dismiss' && (
            <span className="text-[10px]" style={{ color: 'var(--t-muted)' }}>{t('noAction')}</span>
          )}
          {notif.reward && notif.reward > 0 && (
            <span className="text-[10px] font-bold" style={{ color: 'var(--t-accent)' }}>
    ...
    <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[90] flex flex-col gap-2 max-w-md w-full px-4">
      {items.map((item) => (
        <div key={item.key} className="flex items-start gap-3 px-4 py-3 rounded"
          style={{ backgroundColor: 'rgba(10,10,14,0.95)', border: '1px solid var(--t-border)', boxShadow: '0 4px 20px var(--t-shadow)' }}>
          <div className="flex-1">{item.content}</div>
          <button onClick={() => dismiss(item.key)}
            className="text-[10px] px-1.5 py-0.5 shrink-0 cursor-pointer" style={{ color: 'var(--t-muted)', border: '1px solid var(--t-border-strong)' }}>
            X
          </button>
        </div>
      ))}
