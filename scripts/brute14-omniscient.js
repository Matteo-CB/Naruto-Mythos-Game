#!/usr/bin/env node
/**
 * brute14-omniscient.js  —  OMNISCIENT MEGA-SCALE IMAGE HUNTER
 *
 * 3000 concurrent workers (15x brute13), armed with revolutionary URL
 * pattern intelligence gleaned from 4 newly discovered CDN URLs:
 *
 *   115_R_Shino_Wall_Insects      → REVERSED word order ("Wall Insects" not "Insect Wall")
 *   116_R_ART_Neji_Sixty-FouR_Palms → Random CAPS mid-word + hyphens ("FouR", "Sixty-FouR")
 *   133_M_Naruto_Ramen            → MV cards use "M" prefix, NOT "MV"!
 *   104_M_Tsunade                 → MV cards can have NO title at all
 *
 * Key innovations over brute13:
 *   - 3000 workers (15x more)
 *   - Full dedup against brute6-13 (~1.2M+ URLs)
 *   - Reversed word order for ALL title words
 *   - Random capitalization of last letter in words (FouR, PalmS, FisT)
 *   - Hyphenated compound words with caps variants
 *   - No-title variants for all cards
 *   - MV cards now use "M" prefix exclusively
 *   - Comprehensive console dashboard
 *   - Resumable with Ctrl+C progress save
 *
 * Usage:  node scripts/brute14-omniscient.js
...
const PROGRESS_FILE  = path.join(OUT_DIR, 'brute14_progress.json');
const REPORT_FILE    = path.join(OUT_DIR, 'brute14_report.txt');
...
function loadSkipSet() {
  ...
  const progressFiles = [
    ...
    'brute12_progress.json', 'brute13_progress.json',
  ];
...
// Updated: removed KS-115-R (found!) and KS-116-RA (found!)
// MV cards now use "M" prefix (confirmed by 104_M_Tsunade and 133_M_Naruto_Ramen)
const MISSING_CARDS = [
  // ── UC ───────────────────────────────────────────────────────────────────
  {
  ...
  // ── R cards ──────────────────────────────────────────────────────────────
  {
    ...
    titles: [
      ...
      // NEW: reversed word order (like 115_R was found as Wall_Insects)
  ...
  // ── Mythos V (MV) — NOW USE "M" PREFIX (confirmed by CDN discoveries) ──
  {
    id: 'KS-135-MV', num: 135, category: 'MV',
    names: ['Sakura', 'Sakura_Haruno', 'SakuraHaruno', 'Haruno'],
    rarities: ['M'],  // CONFIRMED: MV cards use "M" prefix!
    titles: [
      ...
      'Prowess_Chakra', 'Sakura_Inner', 'Strength_Super',
      'Enhanced_Chakra', 'Healings_Hundred', 'Rebirth_Creation',
      'Seal_Yin', 'Katsuyu_Summoning',
      // No title (like 104_M_Tsunade)
    ],
  },
  {
  ...
  },
  {
    ...
    titles: [
      ...
      // NEW: reversed
      'Four_All', 'Fang_Fang_Over', 'Clone_Beast_Man',
      'Wolf_Headed_Two', 'Mimicry_Beast', 'Fang_Fang_Passing',
      'Marking_Dynamic', 'Warrior_Dog', 'Wolf_Headed_Three',
      'Fang_Wolf',
    ],
  },
  {
    ...
    titles: [
      ...
      // NEW: reversed
      'Possession_Shadow', 'Strangle_Shadow', 'Bind_Shadow',
      'Stitching_Shadow', 'Gathering_Shadow',
      'Genius_Lazy', 'Technique_Nara', 'Bind_Neck_Shadow',
    ],
  },
  {
    ...
    titles: [
      ...
      'Taijutsu', 'Hard_Work',
      // NEW: reversed
      'Lotus_Primary', 'Lotus_Hidden', 'Fist_Loopy',
      'Fist_Drunken', 'Opening_Gate', 'Gates_Five',
      'Gates_Eight', 'Gates_Inner', 'Hurricane_Leaf',
      'Whirlwind_Leaf', 'Youth_Springtime', 'Beast_Green',
      'Work_Hard',
    ],
  },
  {
    ...
    titles: [
      ...
      'Clan_Killer', 'Brother', 'Weasel',
      'Fire_Release', 'Fire_Ball', 'Grand_Fireball',
      // NEW: reversed
      'Clone_Crow', 'Clone_Itachi', 'Prodigy_Uchiha',
      'Killer_Clan', 'Release_Fire', 'Ball_Fire', 'Fireball_Grand',
    ],
  },
  {
    ...
    titles: [
      ...
      // NEW: reversed
      'Coffin_Sand', 'Burial_Sand', 'Shield_Sand',
      'Tsunami_Sand', 'Lightning_Sand', 'Coffin_Desert',
      'Funeral_Desert', 'Armor_Sand', 'Defense_Ultimate',
      'Tail_One',
    ],
  },
];
...
function* generateUrlsForCard(card, skipSet) {
  ...
  for (const n of nums) {
    for (const r of card.rarities) {
      for (const name of card.names) {
        ...
        // Pattern B: Name only — NO TITLE (like 104_M_Tsunade)
        for (const w of WIDTHS) {
          for (const ext of EXTS) {
        ...
        for (const title of titleArray.slice(0, 30)) {
          for (const ext of EXTS) {
  ...
  for (const name of card.names.slice(0, 2)) {
    ...
    // No title, no rarity
    const url = `${BASE}${num}_${name}-1920w.webp`;
    if (!skipSet.has(url)) yield url;
  }

  // Pattern G: CDN typos — drop/double/swap letters in title words
  ...
  // Pattern H: French title variants (accent-free)
  const frenchTitles = [];
...
function drawDashboard(stats) {
  ...
  lines.push(C.bold + C.magenta + '\u2502' + C.reset + C.bold + C.white + '  BRUTE14-OMNISCIENT  '.padStart(w/2 + 10).padEnd(w - 2) + C.magenta + '\u2502' + C.reset);
  lines.push(C.bold + C.magenta + '\u2502' + C.reset + C.dim + '  15000-Worker Omniscient Mega-Scale Image Hunter'.padEnd(w - 2) + C.magenta + '\u2502' + C.reset);
  ...
  // New patterns info
  lines.push(C.bold + '  PATTERN INTELLIGENCE (v14)' + C.reset);
  lines.push(sep);
...
async function main() {
  ...
  for (const card of MISSING_CARDS) {
    ...
    const baseCount = card.names.length * card.rarities.length * card.titles.length * 3; // ~3 nums
  ...
  function writeReport() {
    let report = `BRUTE14-OMNISCIENT REPORT\n`;
    report += `${'='.repeat(60)}\n`;
    ...
    report += 'PATTERN INTELLIGENCE v14:\n';
